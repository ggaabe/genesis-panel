...
function computeStatusAndMutations(
  m: Mission,
  state: RootState,
  dispatch: any
) {
  ...
  if (m.status === "queued" && elapsed >= 5_000) {
    nextStatus = "running";
    if (!startedAt) startedAt = m.createdAt + 5_000;
  }
  if (
    (m.status === "running" || nextStatus === "running") &&
    elapsed >= 15_000 &&
    (!completedAt || m.status === "running")
  ) {
  ...
  if (
    (updated.status === "running" ||
      (prevStatus !== "running" && nextStatus === "running")) &&
    second % 2 === 0
  ) {
    const log: Log = {
      id: nanoid(),
    ...
    if (existing === 0) {
      const art: Artifact = {
        ...
        filename: `${updated.name
          .replace(/\s+/g, "_")
          .toLowerCase()}_${updated.id.slice(0, 6)}.json`,
...
export const mockBaseQuery: BaseQueryFn<Request, unknown, unknown> = async (
  ...
  try {
    ...
    if (url === "/missions" && method === "GET") {
      ...
      const missions = Object.values(state.mockDb.missions).map((m) =>
        computeStatusAndMutations(m, state, api.dispatch)
      );
    ...
    if (url === "/missions" && method === "POST") {
      const id = `m_${Date.now().toString(36)}_${Math.floor(
        rng() * 1e6
      ).toString(36)}`;
    ...
    if (logsMatch && method === "GET") {
      ...
      const logs = (state.mockDb.logsByMission[id] || []).filter(
        (l) => l.ts > afterTs
      );
      return { data: logs };
    }
    ...
    if (retryMatch && method === "POST") {
      ...
      api.dispatch(
        pushLog({
          ...
          ts: now(),
          level: "info",
          message: "Retry requested. Mission queued",
        } as Log)
      );
      return { data: retried };
    }
    ...
    return {
      error: { status: 400, data: `Unknown route ${method} ${url}` },
    } as any;
  } catch (e: any) {
