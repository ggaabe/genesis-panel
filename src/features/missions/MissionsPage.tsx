...
export default function MissionsPage() {
  const {
    ...
    refetch,
    isFetching,
  } = useGetMissionsQuery(undefined, { pollingInterval: 1000 });
      ...
      <Group justify="space-between">
        ...
        <Group>
          <ActionIcon
            ...
            onClick={() => refetch()}
            loading={isFetching}
            data-testid="btn-refresh-missions"
          >
            <IconRefresh size={16} />
          </ActionIcon>
          <Button onClick={open} data-testid="btn-create-mission">
            Create Mission
          </Button>
        </Group>
      </Group>
          ...
          <Table stickyHeader highlightOnHover>
            ...
            <Table.Tbody>
              {filtered.map((m: Mission) => (
                ...
                >
                  <Table.Td>
                    <Badge variant="light">{m.id.slice(0, 8)}</Badge>
                  </Table.Td>
                  <Table.Td>{m.name}</Table.Td>
                  <Table.Td data-testid={`mission-status-${m.id}`}>
                    <MissionStatusBadge status={m.status} />
                  </Table.Td>
                  <Table.Td data-testid={`mission-started-${m.id}`}>
                    {m.startedAt ? fmtDate(m.startedAt) : "-"}
                  </Table.Td>
                  <Table.Td data-testid={`mission-duration-${m.id}`}>
                    {m.startedAt
                      ? fmtDuration(
              ...
              {filtered.length === 0 && (
                <Table.Tr>
                  <Table.Td colSpan={5}>
                    <Text c="dimmed">No missions</Text>
                  </Table.Td>
                </Table.Tr>
              )}
            </Table.Tbody>
